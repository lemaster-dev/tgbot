...
async def start(message: types.Message):
    ...
    await message.answer('Вітаю, натисніть на кнопку та надайте інформацію для замовлення', reply_markup=markup)


@dp.message_handler(content_types=['web_app_data'])
async def web_app(message: types.Message):
    res = json.loads(message.web_app_data.data)
    await message.answer(f'Ваші дані: Імʼя: {res ["name"]}. Email: {res["email"]}. Телефон: {res["phone"]}. Місто: {res ["town"]}. Адреса: {res ["address"]}. Тепер введіть команду "/pay" для оплати')


@dp.message_handler(commands=['pay'])
async def start(message: types.Message):
    await bot.send_invoice(message.chat.id, 'Ваше замовлення', 'Новорічні іграшки', 'invoice', config.PAYMENT_TOKEN, 'USD', [types.LabeledPrice('Замовлення', 1 * 100)])
